/*!
  ...
  This sample program calculates the free energy of the two-dimensional Ising model by
  using a tensor renormalization group method based on higher-order singular
  value decomposition (HOTRG).
...
inline double Hotrg::free_energy() const {
  return -temp*(log_factor+log(trace(a, Axes(0,1), Axes(2,3))))/exp(log_n_spin);
}


inline double Hotrg::n_spin() const {
  return exp(log_n_spin);
}
