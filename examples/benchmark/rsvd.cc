...
double singular_value(size_t i) {
  return std::pow(1.0+i, -2.0);
}
...
ptensor test_tensor(size_t n) {
  ...
  return transpose(tensordot(tensordot(U1, D, Axes(2), Axes(0)),
                                   U2, Axes(2), Axes(2) ),
                   Axes(1,3,0,2));
}
...
int main(int argc, char **argv) {
  ...
  unsigned int seed;
  if (argc < 2) {
    if (mpiroot) std::cerr << "Usage: a.out [N [target_rank [oversamp [seed]]]]\n"
  ...
  if(mpiroot) {
    ...
    std::cout << "# index s_exact s_full s_rsvd s_rsvd_f s_full-s_rsvd s_full-s_rsvd_f\n";
