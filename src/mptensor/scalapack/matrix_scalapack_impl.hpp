...
template <typename C>
Matrix<C>::Matrix()
  : grid(MPI_COMM_WORLD), desc(9) {
  init(0, 0);
};

template <typename C>
Matrix<C>::Matrix(const MPI_Comm& comm)
  : grid(comm), desc(9) {
  init(0, 0);
};
...
template <typename C> inline
const MPI_Comm& Matrix<C>::get_comm() const { return grid.comm; }

template <typename C> inline
int Matrix<C>::get_comm_size() const { return grid.mpisize; }

template <typename C> inline
int Matrix<C>::get_comm_rank() const { return grid.myrank; }

template <typename C> inline
const int* Matrix<C>::descriptor() const { return &(desc[0]); }

// template <typename C> inline
// int Matrix<C>::blacs_context() const { return desc[1]; }

template <typename C> inline
int Matrix<C>::n_row() const { return desc[2]; }

template <typename C> inline
int Matrix<C>::n_col() const { return desc[3]; }
...
template <typename C> inline
int Matrix<C>::get_lld() const { return desc[8]; }
...
#pragma omp for
    for(size_t l_row=0;l_row<size_row;++l_row) {
      global_row[l_row] = BLOCK_SIZE * ((l_row/BLOCK_SIZE) * nprow + myprow) + (l_row%BLOCK_SIZE);
    }
#pragma omp for
    for(size_t l_col=0;l_col<size_col;++l_col) {
      global_col[l_col] = BLOCK_SIZE * ((l_col/BLOCK_SIZE) * npcol + mypcol) + (l_col%BLOCK_SIZE);
    }
  }
...
#pragma omp for
    for(size_t myprow=0;myprow<nprow;++myprow) {
      ...
      if (myprow<extra_blocks) locr += BLOCK_SIZE;
      else if (myprow==extra_blocks) locr += size_row%BLOCK_SIZE;
...
void replace_matrix_data(const Matrix<C>& M,
  ...
  /* Send and Recieve */
  mpi_wrapper::alltoallv(send_pos, send_counts, send_displs,
                         recv_pos, recv_counts, recv_displs, comm);
  mpi_wrapper::alltoallv(send_value, send_counts, send_displs,
                         recv_value, recv_counts, recv_displs, comm);
...
void sum_matrix_data(const Matrix<C>& M,
  ...
  for(int step=0;step<mpisize;++step) {
    ...
    if(dest == mpirank) {
      ...
      MPI_Status status;
      int tag = step;
      MPI_Sendrecv(&send_size, 1, MPI_INT, dest, tag,
                   &recv_size, 1, MPI_INT, source, tag,
                   comm, &status);
      ...
      MPI_Sendrecv( &(send_pos[0]), send_size, MPI_UNSIGNED_LONG, dest, tag,
                    ...
                    comm, &status);
      mpi_wrapper::send_recv_vector(send_value, dest, tag,
                                    recv_value, source, tag,
                                    comm, status);
